...
- 完成真实适配与训练框架初版，聚焦测试 `9 passed`。
- 完成 stride=8、单种子、单 epoch 真实 smoke test，三种模型均产出验证/测试指标和
  机器可读结果；结果仅用于工程验证，不纳入比赛结论。
- 完成 stride=4、三种子正式训练：MCR 与无约束 MoE 均未超过匹配 HGB，且神经模型
  校准明显较差。下一步仅补验证期概率校准和自动停止结论，不继续堆模型结构。
- Platt 校准首个聚焦测试发现近乎无正则的逻辑回归在小验证集上概率饱和为 1；
  已改为标准 L2 正则校准，避免完全分离。
- 正式校准重跑完成并保持 `research_only_not_adopted`；发现校准输入尺度和未定义 FAR
  的制品问题，已改为标准化 logit、NaN-aware 汇总和严格 JSON `null`。
- 标准化后 `lbfgs` 在当前 sklearn 版本仍输出内部矩阵溢出 warning，但拟合参数有限；
  校准器进一步改为适合一维二分类的 `liblinear`，不改变验证期专用原则。
- 使用 `liblinear` 完成最终无告警重跑；严格 JSON 可解析且无非有限浮点，自动结论为
  `research_only_not_adopted`。
- 新增结果制品合同测试和实现文档：冻结三种子、验证期校准、测试不调参、代理标签边界
  及 MCR 不采用结论；阶段 24 完成。
- 结果报告合同首次因目标短语跨 Markdown 换行失败；已改为规范化空白后的语义检查，
  不修改科学边界或实验结果。
- 用户批准实施比赛版历史预警应用；启动阶段 25。
- 已锁定 React + TypeScript、FastAPI、双语、多路由、SQLite、本地离线运行、
  确定性分析加可选 DeepSeek、预置与自由查询并存的实施方案。
- 用户明确要求暂不在新版产品中展示 MCR；旧 MCR 代码保留为未来研究资产。
- 已读取网站构建规范；由于本项目已有工程结构、没有 `.openai/hosting.json` 且目标为
  本地一键启动，本阶段使用既有仓库的 capability path，不创建托管站点。
- 完成比赛应用后端首个闭环：FastAPI合同、真实工具适配、三图层网格场、SQLite审计、
  确定性助手、CAP安全检查、双语HTML/JSON报告和Legacy静态挂载。
- 新增真实数值回归测试，锁定麦加 2025-08-04 高温概率、前一日输入、模型—规则结论、
  80×110概率场和CAP Exercise状态。
- 后端聚焦测试 `6 passed`；全仓测试 `103 passed`，仅1个FastAPI TestClient弃用提示。
- 完成React + TypeScript五路由前端：预警台、分析、证据、助手、报告；支持中英切换、
  预置/自由查询、历史恢复、Canvas网格场、报告与CAP下载。
- 前端Vitest `3 passed`，生产构建成功；分享封面按成品视觉生成并接入Open Graph元数据。
- 补齐可选DeepSeek真实调用：仅发送冻结结果和证据，超时/失败回退确定性分析，
  不改变预测值；新增成功与失败回退测试。
- 使用真实本地服务完成浏览器验收：麦加高温演练生成90%风险简报、80×110概率场、
  可靠性指标、证据子图和HTML/JSON报告；Legacy入口可访问。
- 内置浏览器无法设置真实390px视口，已记录该限制；桌面无横向溢出，移动端由
  620px/900px媒体查询和合同测试保障。
- 最终聚焦测试 `10 passed`，全仓测试 `107 passed`；前端测试与最终生产构建均通过。
- 用户认为深绿色前端配色与布局压抑；启动阶段 26，视觉方向改为明亮的公共气象服务
  工作台，同时保留全部真实数据、推理、审计、证据和报告能力。
- 完成全站浅色视觉系统和首页布局重排，更新网格场调色、证据图谱、助手、报告中心与
  390px/900px/1120px 响应式规则；前端测试 `3 passed`，生产构建成功。
- 新增浅色视觉合同测试；后端/前端聚焦测试 `8 passed`。
- 完成真实浏览器验收：1440px 首页主次清晰，390px 无横向溢出；五个页面均无横向
  溢出且控制台无错误。更新分享封面为新版明亮视觉。
- 全仓适用测试通过：`108 passed`，仅保留 1 个既有 FastAPI TestClient 弃用提示；
  阶段 26 完成。
- 用户质疑各页面能力是否真实，并反馈字号不舒服；启动阶段 27，核对真实推理、规则、
  人工证据、助手和报告的实现来源，确认需要在产品内直接展示分层而非只在说明中解释。
- 完成阶段 27：五页分别标注“本地模型按次重算 / 模型与规则派生分析 / 人工维护证据库 /
  确定性模板解读 / 运行结果动态生成”，助手改名“结果解读”，并解释已保存结果是
  SQLite 中只读的预测 JSON；全站正文与辅助文字字号上调。
- 修复 650px 助手双栏拥挤，900px 以下改为上下布局；浏览器实测 1440px、650px、
  390px 均无横向溢出，390px 正文 15px、底部导航 10px。
- 用户进一步指出新版不如旧版好看，证据页没有真正展示关系图谱，“结果解读”也缺少明确
  任务。启动阶段 28，核心方向从边界文案微调转为证据网络和决策简报的结构性重做。
- 完成阶段 28：证据页改为深色可交互网络画布，真实绘制观测—灾种、机制—灾种、
  文献—机制三类关系；节点点击可在审计面板查看来源、类型和核验状态。
- “结果解读”更名“决策简报”，默认生成结论、驱动因素、模型—规则复核、适用限制和
  分析/图谱/报告入口，自然语言追问降为末尾可选功能。
- 前端测试 `3 passed`、合同测试 `7 passed`、生产构建通过；真实浏览器在当前比赛窗口
  完整显示关系网络，节点审计交互验证通过，决策简报无横向溢出。
- 用户要求使用新安装的 `ui-ux-designer` 重新设计全部页面；启动阶段 29。该社区技能仅含
  主 `SKILL.md`，引用的可选 `resources/implementation-playbook.md` 未随包安装，已记录并
  按主技能的用户任务、令牌化设计系统、可访问性和验证流程继续。
- 完成阶段 29：建立语义设计令牌、900px底部导航断点、全局当前演练上下文和直接任务型
  页面标题；五页分别优化首屏主流程、事件摘要、图谱选择反馈、决策信息密度和制品工作台。
- 新增 `docs/competition_app_design_system.md`，记录目标用户、信息架构、令牌、响应式、
  可访问性和比赛验收合同；新增对应前端合同测试。
- 真实浏览器完成656px比赛窗口逐页视觉验收和390px五路由溢出/控制台/触控目标验收；
  五页滚动宽均为375px且无控制台错误，三个简报链接触控高度修正为32px。
- 用户指出语言切换后仍存在中英文混排；启动阶段 30，按 `ui-ux-designer` 的国际化一致性
  要求审计全部用户可见文案、后端说明和领域标识。
- 完成阶段 30：集中补齐界面文案，并为风险等级、一致性结论、指标、机制、证据核验状态
  和报告类型增加中英文领域映射；中文不再直接显示后端英文机制说明或规则备注。
- 报告中心只显示当前语言的固定材料，城市选择不再“中文 / English”并排；技术标准缩写
  仍按设计合同保留。
- 前端 `6 passed`、前端合同 `10 passed`、生产构建通过；真实浏览器在中英文五路由逐页
  检查均无语言串线和横向溢出。全仓测试受当前环境缺少 `xarray`/`h5py` 阻塞；不依赖
  这些可选包的其余测试为 `66 passed, 1 skipped`，另3个真实适配器测试同样缺少xarray。
- 用户要求整理仓库代码架构并确认多个应用的关系；启动阶段31。先盘点实际入口和依赖，
  再确定唯一比赛主应用、研究原型和Legacy Archive边界，避免直接搬动模型或历史证据。
- 完成阶段31：新增 `APPLICATIONS.md` 作为应用生命周期唯一入口，明确比赛React+FastAPI
  是唯一当前产品，MAZU Atlas为冻结研究原型，`warning_demo`页面为Legacy Archive且其
  模型/数据/工具仍是主应用科学资产。
- 更新README目录地图、比赛/研究原型文档、领域词汇与ADR 0006；研究原型默认端口从
