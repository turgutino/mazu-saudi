...
RISK_CONFIGS: dict[str, HazardRiskConfig] = {
    "heavy-rain": HazardRiskConfig(
        hazard="heavy-rain", hazard_label="暴雨", hazard_label_en="Heavy Rain", sensitivity_key="flash_flood",
        ...
        indicator_rules=(
            IndicatorRule("daily-precip-yellow", "日降水黄色阈值", "Daily Precip Yellow Threshold", "daily_precip", ">=", 25, 1),
            IndicatorRule("daily-precip-orange", "日降水橙色阈值", "Daily Precip Orange Threshold", "daily_precip", ">=", 40, 1),
            IndicatorRule("cape-convective", "CAPE强对流阈值", "CAPE Strong Convection Threshold", "cape", ">=", 2000, 2),
        ),
    ),
    "extreme-heat": HazardRiskConfig(
        hazard="extreme-heat", hazard_label="极端高温", hazard_label_en="Extreme Heat", sensitivity_key="heatwave",
        probability_thresholds=(
            ProbabilityThreshold("yellow", 0.55, 2),
        ...
        indicator_rules=(
            IndicatorRule("t2m-yellow", "高温黄色阈值", "Heat Yellow Threshold", "t2m", ">=", 45, 1),
            IndicatorRule("t2m-orange", "高温橙色阈值", "Heat Orange Threshold", "t2m", ">=", 48, 2),
        ),
    ),
    "flash-flood": HazardRiskConfig(
        hazard="flash-flood", hazard_label="山洪", hazard_label_en="Flash Flood", sensitivity_key="flash_flood",
        probability_thresholds=(
        ...
        indicator_rules=(
            IndicatorRule("daily-precip-yellow", "日降水黄色阈值", "Daily Precip Yellow Threshold", "daily_precip", ">=", 30, 1),
            IndicatorRule("daily-precip-red", "红色预警降水阈值", "Red Alert Precip Threshold", "daily_precip", ">=", 50, 2),
            IndicatorRule("cape-convective", "CAPE强对流阈值", "CAPE Strong Convection Threshold", "cape", ">=", 2000, 2),
        ),
    ),
    "dust-storm": HazardRiskConfig(
        hazard="dust-storm", hazard_label="沙尘暴", hazard_label_en="Dust Storm", sensitivity_key="dust_storm",
        probability_thresholds=(
        ...
        indicator_rules=(
            IndicatorRule("wind-10m-yellow", "沙尘暴黄色阈值", "Dust Storm Yellow Threshold", "wind_10m", ">=", 20, 1),
            IndicatorRule("wind-10m-orange", "沙尘暴橙色阈值", "Dust Storm Orange Threshold", "wind_10m", ">=", 28, 2),
        ),
    ),
}
