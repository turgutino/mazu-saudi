...
| 风险研判工程 | 实验锁、真值隔离、四状态、Schema/SHA、失败降级 | 小样本结果冒充稳定泛化、无许可证源码 |
| Analog/多时效工程 | Analog Ensemble、可靠性图、时效衰减、空间诊断 | 随机格点切分、测试集校准、未知未来填负例 |
| 环流模板工程 | 小样本轻量基线、机制原型思想、失败记录 | 全年未来模板、目标日场和不可复现成绩 |

这些资产如何进入当前仓库、哪些只作为基线以及按什么顺序实施，见[参考资产迁移蓝图](reference_asset_migration_blueprint.md)。
