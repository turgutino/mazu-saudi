...
- MAZU Atlas 已实现现代化本地 API 和页面，但当前是
  `mode=demo`、`scientific_evidence=false` 的确定性演示载荷。
...
| MAZU Atlas | API、页面和 demo backend 完成 | 产品交互原型，非真实预报 |
...
- 不要在来源、许可和时间语义未解决时临时融合外部数据。新增数据会扩大合规、
  对齐和泄漏风险。
- 不要把 MCR-Precip 的合成 Atlas 概率与 `warning_demo` 的真实回顾性结果混在一张图中。
...
→ MAZU Atlas 展示下一代 MCR 产品设计（明确 DEMO）
```
