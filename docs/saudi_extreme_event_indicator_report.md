...
| 灾害/场景 | 关键指标组 | 解释重点 |
|---|---|---|
| 暴雨和山洪 | 降水、对流降水比例、CAPE、IVT、低层风、水汽辐合、短时卫星降水、`flash_flood_risk` | 判断是否具备强水汽、强对流和强降水叠加条件 |
| 极端高温和热健康风险 | 2 米气温、最高/最低温、热指数、VPD、辐射收支、热胁迫代理指标、SST | 判断热力背景、干热程度、夜间降温能力和沿海热湿环境 |
...
| `apparent_temp_c` | 体感温度 | `aptmp - 273.15` | degC | 当前有少数日期被填充值污染，需质控后使用 |
...
| 指标 | 含义 | 计算方法 | 单位 | 使用解释 |
|---|---|---|---|---|
| `cape` | 对流有效位能 | `cape` | J kg-1 | 越高代表潜在对流能量越强 |
| `cin` | 对流抑制能量 | `cin` | J kg-1 | 越负代表抑制越强，接近 0 时更容易触发对流 |
| `surface_lifted_index` | 地面抬升指数 | `lftx` | K | 越低越不稳定 |
| `best_lifted_index` | 最佳四层抬升指数 | `lftx4` | K | 越低越有利于强对流 |
...
| `rh2m` | 364 | 32.54 % | 48.02 % | 57.31 % | 100.1 % @ 20250212 |
...
| `cape` | 293 | 202 J/kg | 547 J/kg | 686 J/kg | 5196 J/kg @ 20250831 |
...
`vpd_kpa` 经剔除异常派生值后，区域均值约 2.76 kPa，95 分位约 4.46 kPa，最大局地值约 9.42 kPa，表现出强干热蒸发需求。`heat_index_c` 的局地最大值达到 54.67 degC，发生在 2025-08-16，说明沿海或湿热区域的体感热风险可能显著高于单纯气温。

### 4.5 对流、水汽输送和动力背景

`cape` 有效覆盖 293 天，区域均值约 202 J/kg，95 分位约 547 J/kg，但局地最大值达到 5196 J/kg，发生在 2025-08-31。这说明多数日期区域平均对流能量不高，但局地强不稳定环境存在。
...
`vpd_kpa` 和 `apparent_temp_c` 在 2025-01-02、2025-01-30 出现明显异常：
...
这些异常不符合物理意义，说明派生计算前需要先清理 GRIB 缺测值，或者在写出 NetCDF 时保留 `_FillValue` 掩膜。上文分布表已经对 `vpd_kpa` 使用 0-10 kPa 的合理范围做了质控后统计。
