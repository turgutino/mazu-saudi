...
class ComputeIndicatorsTests(unittest.TestCase):
    ...
    def test_compute_period_writes_available_excel_indicators(self):
        with TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertIn("missing_advanced_indicators", saved.attrs)
    ...
    def _write_partitioned_inputs(self, root, period):
        ...
        self._write_nc(
            ds2_dir / f"saudi_ds2_ART_ATM_GLB_0P10_DAY_ANAL_{period}.nc",
            {
                "cape": np.full(shape, 1200.0),
                "cin": np.full(shape, -25.0),
                "sp": np.full(shape, 98000.0),
                "orog": np.full(shape, 250.0),
                "lftx": np.full(shape, -3.0),
                "lftx4": np.full(shape, -4.0),
            },
            coords,
        )
