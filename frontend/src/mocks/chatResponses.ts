...
function contextScoreLabel(context: ChatContext): string {
  return scoreLabel(context);
}

function contextCalibrationLabel(context: ChatContext): string {
  return calibrationLabel(context);
}

export function generateGeneralResponse(userMessage: string): string {
  const message = userMessage.toLowerCase().trim();
  if (containsAny(message, ['开始', '怎么用', '如何使用', '工作台', '发起'])) {
    return '请进入**预测工作台**，先选择实时预测或2025历史回放，再选择区域、灾种和系统实际支持的模型。历史回放当前只支持24小时T+1日模型。';
  }
  if (containsAny(message, ['灾种', '支持', '类型'])) {
    return '系统当前覆盖**极端高温、山洪、沙尘暴和强降雨**。2025历史回放有高温、山洪和沙尘暴模型；四个灾种都有使用第三方逐小时预报聚合特征的实时HGB后处理模型。';
  }
  if (containsAny(message, ['解释', '图谱', '结果'])) {
...
export function generateResponse(userMessage: string, context: ChatContext): string {
  ...
  if (containsAny(message, ['可信', '信度', '靠谱', '准确', '不确定', '置信', '边界', '模糊'])) {
    return [
      `本次结果是**${contextScoreLabel(context)} ${score}**，校准状态为**${contextCalibrationLabel(context)}**。`,
      `启发式判别模糊度为 **${(context.ambiguity * 100).toFixed(0)}/100（${ambiguityLabel(context.ambiguity)}）**。它只反映模型分数接近决策中点的程度，不是±误差或统计置信区间。`,
      `模型为 **${context.modelName} ${context.modelVersion}**，输入层级为 **${context.dataTier}**。因此该结果应与后续更新及官方预警共同使用。`,
    ].join('\n\n');
  }

  if (containsAny(message, ['分数', '概率', '为什么', '贡献', '因素'])) {
    const features = context.featureSummaries.length
      ? context.featureSummaries.map((item) => `- ${item}`).join('\n')
      : '- 当前记录没有可验证的Tree SHAP归因';
    return `本次**${contextScoreLabel(context)}**为 **${score}**。Tree SHAP在模型raw log-odds尺度上的主要局部归因为：\n${features}\n\n这些贡献解释模型输出，不代表物理因果或概率百分点。`;
  }

  if (containsAny(message, ['风险', '等级', '预警', '阈值', '怎么定'])) {
  ...
  if (containsAny(message, ['历史', '以前', '类似', '案例'])) {
  ...
  if (containsAny(message, ['物理', '机制', '机理', '因果'])) {
  ...
  if (containsAny(message, ['模型', '数据', '输入', '版本', '来源'])) {
  ...
  return `当前记录：${context.regionName} ${context.hazardLabel}，${contextScoreLabel(context)} **${score}**，风险等级 **${context.riskLevel}**。你可以继续询问模型贡献、校准状态、风险规则、机制或历史案例。`;
}
