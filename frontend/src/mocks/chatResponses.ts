...
export function generateGeneralResponse(userMessage: string): string {
  ...
  if (containsAny(msg, ['你好', 'hi', 'hello', '嗨', '在吗', '做什么', '能做什么', '功能', '介绍一下', '你是谁'])) {
    return [
      ...
      '- 在工作台选择区域和灾种，发起预测',
      '- 解读预测结果：概率、风险等级、不确定性',
      '- 解释模型判断：Tree SHAP 原始 log-odds 局部归因、命中规则、物理机制',
      '- 查询相似历史事件',
      '- 在知识图谱中追踪推理证据链',
      '',
      '要开始使用，你可以先去**预测仪表盘**查看最近的预测记录，或者去**预测工作台**发起一次新预测。',
      '',
      '关于预测的可靠性：概率来自训练好的模型，规则来自业务阈值配置，模型解释采用可加和校验的 Tree SHAP，物理机制来自专家知识。SHAP 值是原始 log-odds 贡献，不是概率百分点或物理因果。',
    ].join('\n');
  }
  ...
  if (containsAny(msg, ['解释', '怎么理解', '结果', '怎么看'])) {
    return [
      '预测结果有五种解释维度，互相补充：',
      '',
      '**特征贡献** — 回答「模型为什么给出这个概率？」通过 Tree SHAP 计算每个输入变量对原始 log-odds 的正负贡献，并展示加和校验',
...
export function generateResponse(userMessage: string, context: ChatContext): string {
  ...
  if (containsAny(msg, ['概率', '为什么', '原因', '因素', '贡献', '高', '升高', '上升'])) {
    return [
      ...
      ``,
      `需要注意的是，特征贡献通过 Tree SHAP 计算，反映当前样本中各变量对模型原始 log-odds 的推动或抑制，而非物理因果或概率百分点。可以切换到「特征贡献」Tab 查看完整的正负条形图与加和校验。`,
    ].join('\n');
  }
  ...
  if (containsAny(msg, ['物理', '机制', '机理', '过程', '因果', '为什么会发生', '怎么发生'])) {
    return [
      ...
      ``,
      `模型的局部归因通过 Tree SHAP 的原始 log-odds 正负贡献来体现（见「特征贡献」Tab），物理机制则帮助人类判断气象条件是否相容。两者证据类型不同，不能相互替代。`,
    ].join('\n');
  }
